% [Spectrum, f_vec, err] = nanspectrum1(TS, DT, TIME_UNITS, SEGMENTS, PLOT_OPTION, PLOT_BOOLEAN, INTERPMETHOD, WINDOWMETHOD)
% 
...
% Welch's method.
% 
% This combines the functionality of "nanspectrum" and "nanrotaryspectrum",
% depending on if the input is real or complex.
% 
% Author: Luke Kachelein
% 
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % 
% % IN:  TS =           Time series (row or column vector). If "TS" is
% %                     complex, then the rotary spectral calculation is
% %                     performed and the "SPectrum" output will have two
% %                     columns.
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % 
% % OUT: Spectrum =     The power spectrum , units of [TS_units^2 f_vec_units^-1].
% %                     This is a column vector if the input "TS" is real,
% %                     or a two-column array if "TS" is complex. In the
% %                     latter case, the first column is the clockwise
% %                     spectrum, and the second column is the
...
function [Spectrum, f_vec, err] = nanspectrum1(TS,DT,TIME_UNITS,...
    SEGMENTS,PLOT_OPTION,PLOT_BOOLEAN,INTERPMETHOD,varargin)

if nargin == 0 % Information given for no input:
    disp('For scalar input:')
    disp('[Spectrum, f_vec, err] = nanspectrum(TS, DT, TIME_UNITS, SEGMENTS, PLOT_OPTION, PLOT_BOOLEAN, INTERPMETHOD, WINDOWMETHOD)')
    disp('For vector input:')
    disp('[[S_cw, S_ccw], f_vec, err] = nanrotaryspectrum(u + 1i*v, DT, TIME_UNITS, SEGMENTS, PLOT_OPTION, PLOT_BOOLEAN, INTERPMETHOD, WINDOWMETHOD)')
else
end
...
elseif nargin == 8
    ...
    error('"nanspectrum1" only takes 7 or 8 arguments.')
end
...
elseif ischar(PLOT_OPTION)
    PLOT_OPTION = {PLOT_OPTION,PLOT_OPTION};
else
...
elseif nargin == 8
    ...
    error('"nanrotaryspectrum" only takes 7 or 8 arguments.')
end
